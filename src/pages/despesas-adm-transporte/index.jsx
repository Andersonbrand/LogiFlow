...
function ModalFornecedoresAdmTransporte({ onClose, onSelect }) {
    ...
    const handleSave = async () => {
        ...
        try {
            if (modal?.mode === 'edit') await updateFornecedorAdmTransporte(modal.data.id, payload);
            else await createFornecedorAdmTransporte(payload);
                            ...
                            <div key={f.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition-colors">
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                    ...
                                    <button onClick={() => { setForm({ nome: f.nome, cnpj: f.cnpj||'', telefone: f.telefone||'', email: f.email||'', endereco: f.endereco||'', categoria: f.categoria||'', observacoes: f.observacoes||'' }); setModal({ mode: 'edit', data: f }); }}
                                        className="p-1.5 rounded hover:bg-blue-50"><Icon name="Pencil" size={16} color="#1D4ED8" /></button>
                                    <button onClick={() => handleDelete(f.id)}
                                        className="p-1.5 rounded hover:bg-red-50"><Icon name="Trash2" size={16} color="#DC2626" /></button>
                                </div>
                            </div>
                        ))}
                    </div>
...
function ModalDespesa({ modal, despesasExistentes = [], onClose, onSaved }) {
    ...
    const handleSave = async (forcarApesarDeDuplicata = false) => {
        ...
        if (!forcarApesarDeDuplicata) {
            ...
            // carregado na tela (que pode estar filtrado por mês/categoria).
            let achadosNf = [];
            if (form.nota_fiscal?.trim()) {
                const existentesComMesmaNf = await buscarDespesasComMesmaNf('transporte_despesas_adm', form.nota_fiscal, excluirId);
                achadosNf = existentesComMesmaNf.map(d => ({
                    despesa: d,
                    motivo: `Nota fiscal ${form.nota_fiscal} já lançada${d.fornecedor || d.empresa ? ` para ${d.fornecedor || d.empresa}` : ''}${d.categoria ? ` (${d.categoria})` : ''} em ${d.data_despesa ? FMT(d.data_despesa) : '—'}, valor ${BRL(d.valor)}.`,
                    confianca: 'alta',
                }));
            }

            // Checagem local (fornecedor+valor+data, nº de boleto) sobre o que já está na tela.
            const achadosLocais = detectarPossiveisDuplicatas(despesasExistentes, form, { excluirId });
                                            ...
                                            {editandoCategoria === cat ? (
                                                ...
                                                <>
                                                    ...
                                                    <button type="button" onClick={() => { setEditandoCategoria(cat); setTextoEdicaoCategoria(cat); }}
                                                        className="shrink-0 p-1.5 rounded-md hover:bg-green-50" style={{ color: '#059669' }} title="Renomear"><Icon name="Pencil" size={16} /></button>
                            ...
                            {form.tipo_pagamento === 'boleto' && (
                                <div className="space-y-2">
                                    ...
                                    {(form.boletos || []).map((b, idx) => (
                                        <div key={idx} className="p-2 rounded-lg bg-white border text-xs" style={{ borderColor: '#FED7AA' }}>
                                            <div className="flex items-center gap-2 flex-wrap">
                                                <input value={b.numero_boleto || ''} onChange={setBoletoField(idx, 'numero_boleto')} placeholder={`Boleto ${idx + 1}`}
                                                    className="font-medium text-amber-700 border rounded px-1.5 py-1 w-24 shrink-0" style={{ borderColor: '#FED7AA' }} title="Nº do boleto" />
                                                <input type="date" value={b.vencimento || ''} onChange={setBoletoField(idx, 'vencimento')} className="font-data border rounded px-1.5 py-1 shrink-0" style={{ borderColor: '#FED7AA' }} />
                                                <input type="number" step="0.01" value={b.valor} onChange={setBoletoField(idx, 'valor')} className="font-data font-semibold text-amber-800 border rounded px-1.5 py-1 w-24 shrink-0" style={{ borderColor: '#FED7AA' }} />
                                                <span className={`px-1.5 py-0.5 rounded shrink-0 ${b.pago ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{b.pago ? 'Pago' : 'Pendente'}</span>
                                            ...
                                            <label className="flex items-center gap-1 cursor-pointer mt-1.5" title="Marcar se o boleto já foi entregue ao setor financeiro">
                    ...
                    <div className="flex gap-1 mb-5 p-1 rounded-xl border" style={{ borderColor: 'var(--color-border)', backgroundColor: '#F9FAFB', width: 'fit-content' }}>
                        {[{ id: 'registros', label: 'Registros', icon: 'Receipt' }, { id: 'parcelas', label: 'Parcelas Futuras', icon: 'Clock' }, { id: 'relatorio', label: 'Pago / Em Aberto', icon: 'BarChart2' }, { id: 'boletos', label: 'Boletos', icon: 'CalendarClock' }].map(g => (
                            <button key={g.id} onClick={() => setGuiaDespesas(g.id)}
                                ...
                                <Icon name={g.icon} size={14} color={guiaDespesas === g.id ? '#059669' : 'var(--color-muted-foreground)'} />{g.label}
                            </button>
                        ))}
                    </div>
                                            ...
                                            <td className="px-3 py-3">
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <button onClick={() => setModalBaixa(d)} title="Gerenciar pagamentos"
                                                        ...
                                                        <Icon name="CheckCircle2" size={16} color={((d.boletos || []).length > 0 || (d.parcelas_cartao || []).length > 0) ? "#059669" : "#9CA3AF"} />
                                                    </button>
                                                    {admin && (
                                                        <>
