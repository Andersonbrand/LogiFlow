...
// ✅ MELHORIA: Paginação real implementada
const PAGE_SIZE = 10;
...
const RomaneiosTable = ({ romaneios }) => {
    ...
    const handleSort = (key) => {
        ...
        setPage(1); // volta para página 1 ao ordenar
    };
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-card overflow-hidden">
            ...
            <div className="px-4 md:px-6 py-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-xs text-gray-400 font-caption">
                    {sorted?.length === 0
                        ? "0 romaneios"
                        : `${(currentPage - 1) * PAGE_SIZE + 1}–${Math.min(currentPage * PAGE_SIZE, sorted.length)} de ${sorted.length} romaneios`}
                </span>
                <div className="flex items-center gap-1">
                    <button
                        onClick={() => goToPage(1)}
                        disabled={currentPage === 1}
                        className="p-1.5 rounded hover:bg-slate-100 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                        aria-label="Primeira página"
                    >
                        <Icon name="ChevronsLeft" size={15} color="#6B7280" strokeWidth={2} />
                    </button>
                    <button
                    ...
                    <span className="text-xs text-gray-500 px-2 font-data">{currentPage} / {totalPages}</span>
                    <button
                        onClick={() => goToPage(currentPage + 1)}
                    ...
                    >
                        <Icon name="ChevronsRight" size={15} color="#6B7280" strokeWidth={2} />
                    </button>
                </div>
            </div>
        </div>
