...
// Revogar baixa de um boleto específico
export async function revogarBoletoCaminhao(despesaId, boletoIdx) {
    const { data: current } = await supabase
        .from('caminhoes_despesas')
        .select('boletos')
        .eq('id', despesaId)
...
export async function revogarParcelaCartaoCaminhao(despesaId, parcelaIdx) {
    const { data: current } = await supabase
        .from('caminhoes_despesas')
        .select('parcelas_cartao')
        .eq('id', despesaId)
